...
const config = {
  ...
  h5: {
    ...
    webpackChain(chain) {
      ...
      chain.resolve
        .plugin('NewMultiPlatformPlugin')
        .use(MultiPlatformPlugin, ['described-resolve', 'resolve', {
          chain
        }]);
      chain.module
        .rule("style-multi-loader")
        .test(/\.scss$/)
        .use("@ctrip/multiPlatform/style-multi-loader")
        .loader("@ctrip/multiPlatform/style-multi-loader")
        .options({
          alias: {...(taroConfig.alias || {})},
